...
SNR_streak_data = var(kernel_data(:))/eta_streak_data;
variance_streak_normalized = var(A0{1}(:))/SNR_streak_data;
[streak_noise_matrix, noise_avg, comment] = generate_periodic_noise(size(X0{1},1), 0, 60, 60, variance_streak_normalized);
...
figure;
dispfun = @(Y, A, X, square_size, kplus, idx) showims_multi(Y, A0, X0, A, X, square_size, kplus, idx);

% SBD settings
params.lambda1 = 1e-1 * ones(1, num_kernels);  % regularization parameter for Phase I
params.phase2 = true;
params.kplus = ceil(0.5 * square_size);
params.lambda2 = 5e-2 * ones(1, num_kernels);  % FINAL reg. param. value for Phase II
params.nrefine = 3;
params.signflip = 0.2;
params.xpos = true;
params.getbias = true;
...
% 2. The fun part
[Aout, Xout, extras] = SBD_test_multi(Y, square_size, params, dispfun, A0);

% Save the result
save('SBD-STM_multi.mat', 'Y', 'X0', 'A0', 'Xout', 'Aout', 'square_size', 'num_kernels');
