...
function plotMetricSlice(ax, metrics, metric_field, metric_name, vary_param, fixed_idx, highlighted_dataset, other_alpha)
    data = metrics.(metric_field);
    
    if strcmp(vary_param, 'lambda1')
        x_values = metrics.lambda1_values;
        x_label = '\lambda_1';
        param_indices = (1:length(metrics.lambda1_values)) + ...
            (fixed_idx-1) * length(metrics.lambda1_values);
    else
        x_values = metrics.mini_loop_values;
        x_label = 'mini\_loop';
        param_indices = fixed_idx:length(metrics.lambda1_values):size(data,2);
    end
...
function plotMetricOverview(ax, metrics, metric_field, metric_name, x_param)
    data = metrics.(metric_field);
    % Define more markers and cycle through them
    markers = {'o', 's', 'd', '^', 'v', '>', '<', 'p', 'h'};  % Added more markers
    ...
    % Plot for each parameter value (using same color for all datasets)
    plot_handles = gobjects(length(legend_values), 1);  % Store handles for legend
    
    for j = 1:length(legend_values)
        % Use modulo to cycle through markers if we have more parameters than markers
        marker_idx = mod(j-1, length(markers)) + 1;
        
        % Get data for this parameter value
        if strcmp(x_param, 'lambda1')
            param_indices = (1:length(x_values)) + (j-1) * length(x_values);
        else
            param_indices = j:length(metrics.lambda1_values):size(data,2);
        end
        
        % Plot data from all datasets
        for i = 1:metrics.num_datasets
            y_values = data(i,param_indices);
            h = plot(ax, x_values, y_values, markers{marker_idx}, ...
                'LineWidth', 1.5, ...
                'MarkerSize', 8, ...
                'Color', [0 0 0], ...  % Black color for all
                'HandleVisibility', 'off');  % Hide from legend except last one
            
            if i == metrics.num_datasets  % Only use last plot for legend
                plot_handles(j) = h;
    ...
    % Add legend showing only parameter values (once per parameter)
